            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                        ...
                        {/* === TOMBOL DINAMIS DENGAN ANIMASI KEREN === */}
                        {!checkInTime ? (
                            <button
                                onClick={() => handleOpenQrModal('clock-in')}
                                className="w-full py-4 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white rounded-xl font-bold shadow-lg flex justify-center items-center gap-2
                                           transition-all duration-300 transform hover:-translate-y-1 hover:shadow-xl active:scale-95"
                            >
                            ...
                            <button
                                ...
                                className="w-full py-4 bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 text-white rounded-xl font-bold shadow-lg flex justify-center items-center gap-2
                                           transition-all duration-300 transform hover:-translate-y-1 hover:shadow-xl active:scale-95"
                            >
                                <LogOut size={20} /> Scan Pulang
                            </button>
                        ) : (
                            <div className="w-full py-4 bg-gradient-to-r from-green-100 to-emerald-100 text-green-800 rounded-xl font-bold text-center border-2 border-green-200 
                                            flex items-center justify-center gap-2 transition-all duration-500">
                                <PartyPopper size={20} className="animate-bounce" /> Absensi Selesai Hari Ini!
                            </div>
                        )}
                    </div>
                        ...
                        <div className="fixed inset-0 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
                            <div className="bg-white rounded-xl shadow-xl w-full max-w-sm p-6 relative">
                                ...
                                <form onSubmit={handleQrSubmit} className="space-y-4">
                                    {/* INI BAGIAN YANG HARUS DIUBAH - PASTIKAN KODE INI */}
                                    <input 
                                        type="text" 
                                        value={qrInput} 
                                        onChange={(e) => setQrInput(e.target.value)} 
                                        placeholder="Scan atau Masukkan Token ID Card" 
                                        className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg bg-white 
                                                focus:border-blue-500 outline-none font-mono text-center text-lg
                                    ...
                                    <div className="grid grid-cols-2 gap-3 pt-2">
                                        <button 
                                            ...
                                            className="py-3 rounded-lg border-2 border-gray-200 text-gray-600 font-bold hover:bg-gray-50 transition-colors duration-200"
                                        >
                                            Batal
                                        </button>
                                        <button 
                                            ...
                                            className="py-3 rounded-lg bg-blue-600 text-white font-bold 
                                                    ...
                                                    disabled:bg-blue-300 flex justify-center items-center"
                                        >
                                            {qrModalLoading ? (
                                                <>
